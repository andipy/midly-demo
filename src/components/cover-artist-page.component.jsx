...
const CoverArtistPage = ({ artName, image, flashLeaderboard }) => {
    ...
    return (
        <header className={`position-relative h-xs-20 ${location.pathname.includes('leaderboard-flash') ? 'position-fixed w-100 z-index-5 top-0' : ''}`}>
            ...
            {!location.pathname.includes('leaderboard-flash') && <div className="container w-100 position-absolute-x bottom-avatar-header z-index-2 d-flex-row align-items-center">
                <div className="position-relative avatar-72">
                    <img className={`avatar-72 border-radius-100 ${flashLeaderboard ? 'border-red-6' : 'border-dark-6'}`} src={image} />                       
