...
const CardLeaderboardFan = ({ fanImage, fanUsername, fanPoints, fanPosition }) => {
    ...
    return (
        <article className={`d-flex-row align-items-center j-c-space-between bg-dark-gradient ${pathname.includes('leaderboard-flash') ? 'pb-xs-2 pt-xs-2 pr-xs-4 pl-xs-2 border-radius-100 mb-xs-2' : 'pb-xs-4 pt-xs-4 pr-xs-6 pl-xs-6 border-radius-1 mb-xs-3'}`}>
            <div className="d-flex-row align-items-center w-100">
                <img className="avatar-38 border-radius-100 mr-xs-4" src={fanImage} />
