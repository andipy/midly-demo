...
const CardArtistHighlight = ({ artist, isFollowed, length }) => {
    ...
    return (
        <>
            {length === 1 ?
                <div className='mr-xs-2 position-relative w-100 h-xs-22 overflow-clip border-radius-04' onClick={() => navigate(`/artist/${artist?.slug}/leaderboard`, { state: artist })}>
            ...
            :
                <div className='mr-xs-2 position-relative w-80vw h-xs-22 overflow-clip border-radius-04' onClick={() => navigate(`/artist/${artist?.slug}/leaderboard`, { state: artist })}>
                    <div className='overlay-card bg-dark-overlay-card z-index-1'></div>
                    <img className='h-inherit w-80vw object-fit-cover' src={artist.image} />
                    <div className='d-flex-column position-absolute bottom-5 ml-xs-4 z-index-2'>
                        <h5 className='fsize-xs-2 f-w-500 letter-spacing-1'>{artist.artistName}</h5>
                    </div>
                    {isFollowed &&
                        <div className='overlay-card-followed z-index-3 bg-black-transp50 d-flex-row j-c-center align-items-center gap-0_25em h-inherit'>
                            <img src={IconSuccess} alt='Y!' />
