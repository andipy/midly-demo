...
const CardFollowedArtist = ({ artist }) => {
    ...
    return (
        <div onClick={() => navigate(`/artist/${artist.artistSlug}/leaderboard`, { state : artist })} className={`d-flex-row align-items-center bg-dark-gradient border-radius-1 mb-xs-3 pt-xs-2 pb-xs-2 pl-xs-2 pr-xs-2 gap-0_5em`}>                        
                <img className="followed-artist-img object-fit-cover border-radius-1" src={artist.image} />

                <h5 className="fsize-xs-4 f-w-500">{artist.artName}</h5>

                {artist.artName == 'thasuP' &&
